...
**Creating a log file
set logtype smcl
log using "$do/nepal_earthquake.smcl", replace
...
global wave1 "$data/wave_1"
cap{
	mkdir "$data/wave_1/merged_data"
}
global merged_data "$data/wave_1/merged_data"
*I am using one of the datasets from the folders
********************************************************************************
*************************----------------------------***************************
*-----------------------Using dataset from Round 1-----------------------------*
*************************----------------------------***************************
********************************************************************************

use "$raw_data_1/Section_0.dta"
*Drop the variables not required
drop s00q00c s00q00d s00q01 s00q02b s00q03 s00q07
...
*I will remove vdc and vdc_code
drop vdc s00q02a
...
*************************----------------------------***************************
use "$raw_data_1/Section_1.dta"
*rename the variables
rename s01q00a still_memb
rename s01q00b moved_to
rename s01q00c moved_to_other 
...
rename s02q11b_s_eng missing_reason_other
label var missing_reason_other "Other reason for absent"

rename s02q11c days_didnot_go
rename s02q11d reason_didnot_go
rename s02q11d_s_eng reason_other
label var reason_other "Other Reasons for not going to school "
*saving dataset--------------------------------*
save "$merged_data/education.dta", replace
*************************----------------------------***************************
*----------------------Using Dataset Section_3 Dataset-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_3.dta"
*renaming variables***
...
rename s04q27a firewood_used
rename s04q27b firewood_collected
**saving the file in work directory
...
*I have multiple responses for foodid for each households. I want to convert those foodid as each variables***
gen rice = (foodid==11)
gen flat_rice = (foodid==12)
gen maize = (foodid==13)
gen maize_flour = (foodid==14)

gen wheat_flour = (foodid==15)
gen millet = (foodid==16)
gen barley = (foodid==17)
gen other_grains = (foodid==18)

gen lentil = (foodid==21)
gen horse_gram = (foodid==22)
gen beans = (foodid==23)

gen eggs = (foodid==31)
gen milk = (foodid==32)
gen milk_powder = (foodid==33)
gen curd = (foodid==34)
gen other_milk = (foodid==35)

gen ghee = (foodid==41)
gen veg_oil = (foodid==42)
gen mustard_oil = (foodid==43)
gen other_oil = (foodid==44)

gen potatoes = (foodid==51)
gen onions = (foodid==52)
gen tomatoes = (foodid==53)
gen leafy_veg = (foodid==54)
gen other_veg = (foodid==55)

gen fruits =(foodid==61)
gen dried_nuts=(foodid==62)

gen fish=(foodid==71)
gen mutton=(foodid==72)
gen buff=(foodid==73)
gen chicken=(foodid==74)
gen other_meat=(foodid==75)

gen salt=(foodid==81)
gen other_spices=(foodid==82)

gen sugar=(foodid==91)
gen gud=(foodid==92)
gen sweets=(foodid==93)

gen tea_coffee=(foodid==101)
gen other_nonalcohol=(foodid==102)
gen alcohol=(foodid==111)
gen cigarettes=(foodid==121)
gen tobacco=(foodid==122)
gen other_food=(foodid==131)
gen none=(foodid==998)

e
//Defining a list containing the names of the variables 
local varlist "rice flat_rice maize maize_flour wheat_flour millet barley other_grains lentil horse_gram beans eggs milk milk_powder curd other_milk ghee veg_oil mustard_oil other_oil potatoes onions tomatoes leafy_veg other_veg fruits dried_nuts fish mutton buff chicken other_meat salt other_spices sugar gud sweets tea_coffee other_nonalcohol alcohol cigarettes tobacco other_food none"

collapse (max) `varlist', by(hhid)
...
*save the dataset--------------------------------*
save "$wave1/section_5a.dta", replace

*************************----------------------------***************************
*----------------------Using Dataset Section_5(b) Dataset----------------------*
*************************----------------------------***************************

use "$raw_data_1/Section_5b.dta"
...
save "$wave1/section_5b.dta", replace

*************************----------------------------***************************
*----------------------Merging Dataset 5(a) and 5(b)---------------------------*
*************************----------------------------***************************
use "$wave1/section_5a.dta"

merge m:1 hhid using "$wave1/section_5b.dta"
drop _merge
...
*************************----------------------------***************************
use "$raw_data_1/Section_6a.dta"
*renaming the variables***
rename s06q01a value_month
rename s06q01b value_year
*Saving the dataset--------------------------------*
save "$wave1/section_6(a)", replace

*************************----------------------------***************************
*----------------------Using Dataset Section_6(b) Dataset----------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_6b.dta"
*renaming the variables***
rename s06q02 infreq_exp_year
*saving the dataset--------------------------------*
save "$wave1/section_6(b)", replace

*************************----------------------------***************************
*----------------------Using Dataset Section_6(c) Dataset----------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_6c.dta"
*renaming the variables***
rename s06q03a num_item
rename s06q03b value_item
*saving the dataset--------------------------------*
save "$wave1/section_6(c)", replace

*************************----------------------------***************************
*----------------------Using Dataset Section_6(d) Dataset----------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_6d.dta"
*renaming the variables***
rename s06q04a self_own_item
rename s06q04b self_amt_month 
rename s06q04c self_amt_year
*saving the dataset--------------------------------*
save "$wave1/section_6(d)", replace

*************************----------------------------***************************
*-------------------Merging 6(a) 6(b) 6(c) and 6(d) datasets-------------------*
*************************----------------------------***************************

use "$wave1/section_6(a).dta"
append using "$wave1/section_6(b).dta"
save "$wave1/section_6(a_b).dta", replace

use "$wave1/section_6(a_b).dta"
append using "$wave1/section_6(c).dta"
save "$wave1/section_6(a_b_c).dta", replace
use "$wave1/section_6(d).dta"
merge 1:m hhid using "$wave1/section_6(a_b_c).dta"
drop _merge
...
*************************----------------------------***************************
use "$raw_data_1/Section_9a1.dta"
*rename the variables***
rename s09q02a change_plot
...
*saving the dataset--------------------------------*
save "$wave1/section_9a1.dta", replace

*************************----------------------------***************************
*------------------------Using Dataset Section_9(a2).dta---------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9a2.dta"
*rename the variables***
...
*save the dataset--------------------------------*
save "$wave1/section_9a2.dta", replace
*************************----------------------------***************************
*------------------------Using Dataset Section_9(a3).dta---------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9a3.dta"
*rename the variables***
...
*Saving dataset--------------------------------*
save "$wave1/section_9a3.dta", replace

*************************----------------------------***************************
*------------------------Using Dataset Section_9(a4).dta---------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9a4.dta"
*rename variables***
...
*save the dataset--------------------------------*
save "$wave1/section_9a4.dta", replace

*************************----------------------------***************************
*------------------------Merging dataset section_9a1-9a4-----------------------*
*************************----------------------------***************************
use "$wave1/section_9a1.dta"
append using "$wave1/section_9a2.dta"
save "$wave1/section_9a1_2.dta", replace

use "$wave1/section_9a3.dta"
merge 1:1 hhid using "$wave1/section_9a4.dta"
drop _merge
save "$wave1/section_9a3_4.dta", replace

use "$wave1/section_9a3_4.dta"
merge 1:m hhid using "$wave1/section_9a1_2.dta", keep (match master)
drop _merge
sort hhid
save "$wave1/section_9a1_4.dta", replace
*************************----------------------------***************************
*------------------------Using Dataset Section_9(b1).dta-----------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9b1.dta"
*rename variables***
...
*Save the dataset--------------------------------*
save "$wave1/section_9b1.dta", replace

*************************----------------------------***************************
*------------------------Using Dataset Section_9(b2).dta---------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9b2.dta"
*rename variables***
...
*Save the dataset--------------------------------*
save "$wave1/section_9b2.dta", replace
*************************----------------------------***************************
*-------------------Appending Dataset Section_9(b1)_9(b2)------------------------*
*************************----------------------------***************************
use "$wave1/section_9b2.dta"
append using "$wave1/section_9b1.dta"
sort hhid
save "$wave1/section_9b1_2.dta", replace
*************************----------------------------***************************
*------------------------Using Dataset Section_9(c).dta------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9c.dta"
*rename variables***
...
*Save the dataset--------------------------------*
save "$wave1/section_9c.dta", replace

*************************----------------------------***************************
*------------------------Using Dataset Section_9(d).dta---------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9d.dta"
*rename variables***
...
*save dataset--------------------------------*
save "$wave1/section_9d.dta", replace

*************************----------------------------***************************
*------------------------Using Dataset Section_9(e).dta---------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9e.dta"
...
*Save Dataset----------------------*
save "$wave1/section_9e.dta", replace

*************************----------------------------***************************
*------------------------Using Dataset Section_9(f).dta---------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_9f.dta"
*Rename Variables
...
*Save dataset--------------------------------*
save "$wave1/section_9f.dta", replace

*************************----------------------------***************************
*------------------------Merging datasets 9(a) to 9(f)-------------------------*
*************************----------------------------***************************

use "$wave1/section_9e.dta"
merge 1:1 hhid using "$wave1/section_9c.dta", keep(match using)
drop _merge
save "$wave1/section_9e_c.dta", replace
merge 1:m hhid using "$wave1/section_9a1_4.dta"
drop _merge
save "$wave1/section_9a_e_c.dta", replace
merge m:m hhid using "$wave1/section_9b1_2.dta", keep(match master)
drop _merge
save "$wave1/section_9ac_9b12.dta", replace
use "$wave1/section_9d.dta"
merge m:m hhid using "$wave1/section_9d.dta", keep(match master)
drop _merge
save "$wave1/section_9ac_9b12_9d.dta", replace

merge m:m hhid using "$wave1/section_9f.dta", keep(match master)
drop _merge
save "$merged_data/farming_livestock.dta", replace
*************************----------------------------***************************
*------------------------Use Dataset Section_10.dta----------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_10.dta"
*Rename Variables
...
rename s11q02b_s overseas_destntn_other
...
*************************----------------------------***************************
use "$raw_data_1/Section_12a.dta"
*Rename Variables
...
*Saving Dataset----------------------*
save "$wave1/section_12a.dta", replace

*************************----------------------------***************************
*------------------------Use Dataset Section_12(b).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_12b.dta"
*Rename Variables
...
*Saving Dataset----------------------*
save "$wave1/section_12b.dta", replace

*************************----------------------------***************************
*------------------------Use Dataset Section_12(c).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_12c.dta"
*Rename Variables
rename s12q23 value_asset
rename s12q24 recvd_sources

*Saving Dataset----------------------*
save "$wave1/section_12c.dta", replace
*************************----------------------------***************************
*------------------------Use Dataset Section_12(d).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_12d.dta"
*Rename Variables
...
*Saving Dataset----------------------*
save "$wave1/section_12d.dta", replace
*************************----------------------------***************************
*----------------------Merging datsets in section 12(a-d)----------------------*
*************************----------------------------***************************

use "$wave1/section_12c.dta"
merge m:1 hhid using "$wave1/section_12d.dta", keep(match using)
drop _merge
save "$wave1/section_12cd.dta", replace
use "$wave1/section_12cd.dta"
use "$wave1/section_12b.dta"
merge m:m hhid using "$wave1/section_12a.dta"
drop _merge
save "$wave1/section_12ab.dta", replace
use "$wave1/section_12ab.dta"
merge m:m hhid using "$wave1/section_12cd.dta", keep(match using)
drop _merge
...
*************************----------------------------***************************
use "$raw_data_1/Section_13a.dta"
order member_id, before(giftid)
order house_id, before(giftid)
*Rename Variables
...
rename s13q08_s purpose_others

order s13q06d_eng, before(dist_recipnt)

*Saving Dataset----------------------*
save "$wave1/section_13a.dta", replace

*************************----------------------------***************************
*------------------------Use Dataset Section_13(b).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_13b.dta"
order member_id, before (giftid)
order nyear, before (member_id)
*Rename variables***
rename s13q12 donor_caste
rename s13q13a recpnt_in_roster
rename s13q13b primary_recpnt
...
*Saving Dataset----------------------*
save "$wave1/section_13b.dta", replace

*************************----------------------------***************************
*------------------------Use Dataset Section_13(c).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_13c.dta"
*Rename variables*********
...
rename s13q18b name_ingos

*Saving dataset--------------------------------*
save "$wave1/section_13c.dta", replace

*************************----------------------------***************************
*------------------------Use Dataset Section_13(d).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_13d.dta"
...
*Saving the dataset--------------------------------*
save "$wave1/section_13d.dta", replace
*************************----------------------------***************************
*------------------------Mergig Dataset with section 13------------------------*
*************************----------------------------***************************

//use "$data/section_13a.dta"
use "$wave1/section_13b.dta"
use "$wave1/section_13c.dta"
use "$wave1/section_13d.dta"
merge 1:m hhid using "$wave1/section_13c.dta", keep(match master)
drop _merge
save "$wave1/section_13cd.dta", replace

merge m:m hhid using  "$wave1/section_13b.dta", keep(match master)
drop _merge
save "$wave1/section_13cdb.dta", replace

merge m:m hhid using "$wave1/section_13a.dta", keep(match master)
drop _merge
...
*************************----------------------------***************************
use "$raw_data_1/Section_14a.dta"
...
rename s14q08b grievance_filedwith
...
*Saving Dataset----------------------*
save "$wave1/section_14a.dta", replace

*************************----------------------------***************************
*------------------------Use Dataset Section_14(b).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_14b.dta"
*Renaming variables********************
...
rename s14q12_4b fifthmemb_idkind
rename  s14q13a_eng benft_to_rcv
label var benft_to_rcv " Type of benefit household members are supposed to receive English" /*Here may be some discrepency between the nepali values and english values*/
rename s14q13b_eng benft_act_rcvd
label var benft_act_rcvd "Type of benefit household members actually received English" 
rename s14q14 diff_access
rename s14q15a_1 diff_enroll
rename s14q15a_2 diff_irregpmt
rename s14q15a_3 diff_noinfo
rename s14q15a_4 diff_reach
rename s14q15a_5 diff_other
rename s14q13a_q value_to_rcv
rename s14q13b_q value_act_rcv 

order value_to_rcv, after(benft_to_rcv)
order value_act_rcv, after(benft_act_rcvd)

*Saving Dataset-------------------------*
save "$wave1/section_14b.dta", replace

*************************----------------------------***************************
*------------------------Use Dataset Section_14(c).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_14c.dta"
*Renaming variables********************
...
rename s14q21b food_toget
rename s14q22a wage_act_paid
rename s14q22c kind_act_paid
rename s14q22b earning_if_other
rename s14q23 diff_access_prog
rename s14q24a_1 diff_enroll
rename s14q24a_2 diff_irreg_pmt
rename s14q24a_3 diff_diffwork
rename s14q24a_4 diff_other

*Saving Dataset-------------------------*
save "$wave1/section_14c.dta", replace

*************************----------------------------***************************
*------------------------Merge Dataset for section 14--------------------------*
*************************----------------------------***************************
use "$wave1/section_14a.dta"

merge m:m hhid using "$wave1/section_14b.dta", keep (match master)
drop _merge
save "$wave1/section_14ab.dta", replace

merge m:m hhid using "$wave1/section_14c.dta"
drop _merge
...
*************************----------------------------***************************
use "$raw_data_1/Section_15a.dta"
*Renaming Variables***********************
...
rename s15q06b_1 spend_savings
rename s15q06b_2 pawn_houses
rename s15q06b_3 pawn_animal
rename s15q06b_4 pawn_otherprop
rename s15q06b_5 sold_houseland
rename s15q06b_6 sold_farmanimal
rename s15q06b_7 sold_otherprop
rename s15q06b_8 borrow_prvtbank
rename s15q06b_9 borrow_govbank
rename s15q06b_10 borrow_monylenders
rename s15q06b_11 borrow_friends
rename s15q06b_12 borrow_cooperatives
rename s15q06b_998 otherss
rename s15q06_s other_spec1
rename s15q09_s other_spec2
rename s15q12_s other_spec3

*Saving the Dataset----------------------*
save "$wave1/section_15a.dta", replace
*************************----------------------------***************************
*------------------------Use Dataset Section_15(b).dta-------------------------*
*************************----------------------------***************************
use "$raw_data_1/Section_15b.dta"
*Renaming Variables***********************
...
rename s15q15a emotional_distress_yes
rename s15q15b emotional_distress_no
rename s15q15c assess_family_emotion
rename s15q15d_1 action_prayer
rename s15q15d_2 action_talkfamily
rename s15q15d_3 action_talkfriends
rename s15q15d_4 action_talkvillage
rename s15q15d_5 action_talkrelig
rename s15q15d_6 action_own_commty
rename s15q15d_7 action_other_commty
rename s15q15d_8 action_ngo
rename s15q15d_9 action_donttell
rename s15q15d_10 action_other 
rename s15q15d_s action_specify

rename s15q15e action_taken
rename s15q15f group_help
rename s15q15g_1 know_help_announce
rename s15q15g_2 know_help_commty
rename s15q15g_3 know_help_wordmouth
rename s15q15g_4 know_help_flyers
rename s15q15g_5 know_help_radio
rename s15q15g_6 know_help_tv
rename s15q15g_7 know_help_other
rename s15q15h attend_meeting
rename s15q15i_1 notattend_notime
rename s15q15i_2 noattend_family
rename s15q15i_3 noattend_nowelcome
rename s15q15i_4 noattend_noeffective
rename s15q15i_5 noattend_notrust
rename s15q15i_998 noattend_other
rename s15q15i_999 noattend_noreason

*Saving the dataset--------------------------------*
save "$wave1/section_15b.dta", replace

*************************----------------------------***************************
*------------------------Merge dataset 15a and 15b-----------------------------*
*************************----------------------------***************************
use "$wave1/section_15a.dta"
merge m:1  hhid using "$wave1/section_15b.dta", keep (match using)
drop _merge
...
save "$merged_data/wave_1.dta", replace
